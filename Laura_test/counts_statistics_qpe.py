...
def replace_first_two_chars(lst):
    return ["ii" + item[2:] if len(item) >= 2 else item for item in lst]

def replace_last_two_chars(lst):
    return [item[:2] + "ii" if len(item) >= 2 else item for item in lst]

def replace_char(lst,qubit):
    # qubit = int(qubit) - 3
    # qubit = -1 * qubit
    # print(qubit)
    return [('iiii'[:int(qubit)] + item[int(qubit)] + 'iiii'[int(qubit)+1:]) for item in lst]
            

def eigenvals(qubit, operators, state = None):
    eigenvalues = []
    eigenvectors = []

    if state  == 'Bell':
        if qubit == '0':
            operators = replace_last_two_chars(operators)
        if qubit == '1':
            operators = replace_first_two_chars(operators) 

    if state  == 'Plus':
        operators = replace_char(operators,qubit)    
...
class Operartors_Statistics:
    ...
    def __init__(self, n_qubits, stabilizers, stab_sign, STABILIZER_TO_INDEX, filenames, working_dir_data, state=None, qubit=None, set_loops=None):
        ...
        self.eigenvalues, self.eigenvectors = eigenvals(self.qubit, self.stabilizers,self.state)
        ...
        """
        Each measurement round corresponds to an index that we call loop
        Each loop has a stabilizer that is associated with the measurement basis that we set for that round
        We define that counts_array associated with that same loop and stabilizer
        """
        for i, file in enumerate(filenames):
            ...
            if any(file_label == c for c in self.stabilizers):
                ...
                if set_loops is None:
                    self.loops.append(int((file.split("_"))[1].split(".")[0]))
                else:
                    self.loops.append(i)
    ...
    def get_pass_prob(self, start=0, end=-1, aux_init=True):
        ...
        for i,_ in enumerate(self.stabilizers):
            self.N_positive = self.positive_eigen[i]*self.counts_per_outcome[i]
            self.N_negative = self.negative_eigen[i]*self.counts_per_outcome[i]

            N_positive_error = self.positive_eigen[i]*self.counts_per_outcome_error[i]
            N_negative_error = self.negative_eigen[i]*self.counts_per_outcome_error[i]
    ...
    def get_pass_prob_evolution(self, lim, writting_dir=None):
        ...
        for k in lim:
            p_pass, p_pass_error = self.get_pass_prob(k_last, k, aux_init=False)
    ...
    def plot_pass_prob(self, save_dir=False):
    ...
    def plot_fail_prob(self, save_dir=False):
        measurment_basis = list(map(lambda x: x.upper(), self.stabilizers))
        self.get_expectation_values()
...
"""
Reading and analysing the stats files that are generated in class Operartors_Statistics
"""
class Read_Operator_Statistics:
    def __init__(self, stats_dir):
        ...
        self.p_pass = np.array(p_pass_list, dtype = float)
        self.p_pass_error = np.array(p_pass_error_list, dtype = float)
        self.n_samples = np.array(n_samples_list, dtype = float)
        self.n_samples_error = np.array(p_pass_error_list, dtype = float)
